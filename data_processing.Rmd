...
CHN_GeoSpark_Map <- as_data_frame(CHN_simplified) %>% 
  mutate(chn_map_geom = st_as_text(geometry),
         geometry = NULL)

# Put the Chinese Map into Spark storage.

CHN_GeoSpark_Map <- copy_to(sc, CHN_GeoSpark_Map)
...
CHN_GeoSpark_Map <- mutate(CHN_GeoSpark_Map, chn_geo_wkt = st_geomfromwkt(chn_map_geom))
