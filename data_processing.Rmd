...
```{r time_distribution_testing}

# # Code used for testing when generating the time distribution plot.
# 
...
```{r regex}
s <- "POINT(12.2342342354234234234234232323423 0123)"
sub(".*\\( *(.*?) *[[:space:]].*", "\\1", s)
sub(".*[[:space:]] *(.*?) *\\).*", "\\1", s)
```
...
# # # Some Code Used for Initial Testing. Preserved for later reference
# # 
...
# 0 < as.numeric(str_remove(str_split(A, pattern = " ") %>% pluck(1), pattern = "POINT(")),
#          as.numeric(str_remove(str_split_fixed(A, pattern = " ") %>% pluck(1), pattern = "POINT(")) < 180,
#          -90 < as.numeric(str_remove(str_split(A, pattern = " ") %>% pluck(2), pattern = ")")), 
#          as.numeric(str_remove(str_split(A, pattern = " ") %>% pluck(2), pattern = ")")) < 90
...
sc_res <- st_join(CHN_GeoSpark_Map,
                  WEIBOSCOPE_GEO_WKT,
                  join = sql("st_contains(chn_geo_wkt, weibo_geo_wkt)")) %>% 
...
sdf_num_partitions(CHN_GeoSpark_Map)
sdf_nrow(CHN_GeoSpark_Map)

sdf_register(geo_distribution, name = "geo_distribution")

tbl_cache(sc, "weiboscope_geo_wkt")

tbl_cache(sc, "chn_geospark_map")

tbl_cache(sc)

spark_write_parquet(geo_distribution, path = "~/Documents/gov-1005-projects/chinese-aqi-social-media-sentiment/spark_parquets/geo_distribution.parquet")
